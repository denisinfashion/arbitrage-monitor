...
if go:
    st.session_state["diag_last"] = (text, source, amount, window_h)

text, source, amount, window_h = st.session_state["diag_last"]
...
if rep.missing:
    ...
    st.markdown(
        ...
        "Это значит, что пул либо не попал в справочник, либо отсеян отбором. "
...
with st.expander("Как это считается"):
    st.markdown(
        f"""
Курс плеча берётся из последней котировки пары в выбранном источнике.
Направление не важно: если записано `DAI/USDT`, для плеча `USDT → DAI`
курс переворачивается.
