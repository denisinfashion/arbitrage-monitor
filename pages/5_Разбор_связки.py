...
c4.metric("Налог контрактов", f"−{rep.tax_pct:.2f}%",
          help="Удерживает сам токен при переводе, в цене пула этого "
               "не видно. Проверяется симуляцией обмена; ноль означает "
               "либо честный контракт, либо что проверки ещё не было.")
c5.metric("Итого", "—" if rep.net_pct != rep.net_pct else f"{rep.net_pct:+.2f}%")

if rep.tax_pct > 0:
    st.warning(
        f"Контракты токенов удерживают {rep.tax_pct:.2f}% сверх комиссий "
        f"площадок. Это не проскальзывание и не сбор биржи: процент "
        f"списывает сам токен при переводе, и в кошельке это выглядит "
        f"как недостача. В расчёте выше уже вычтено.",
        icon="🧾",
    )
...
**Налог контракта** — процент, который удерживает сам токен при переводе.
Проверяется симуляцией обмена на форке сети: honeypot.is выполняет покупку
и продажу и сравнивает полученное с обещанным. Разница и есть налог.
Токены, из которых симуляция не смогла выйти, до расчёта не доходят
вовсе — они отсеиваются как непродаваемые. Пустая колонка означает, что
проверки по этому токену ещё не было, а не что налога нет.
