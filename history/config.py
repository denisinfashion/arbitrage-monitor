...
class Settings:
    ...
    dex_venue_pages: int = 6
    """Сколько площадок сети опрашивать при автоопределении.

    Каждая площадка — один запрос из тридцати в минуту, и вместе
    со страницами топа и списком наблюдения мы упирались в лимит
    прямо посреди поиска. Шести хватает: дальше в BNB Chain идут
    площадки с оборотом, на котором арбитраж всё равно неисполним.
    """

    dex_top_pages: int = 5
    """Сколько страниц общего топа сети читать (по 20 пулов на страницу)."""
    ...
    min_pool_reserve_usd: float = float(os.environ.get("ARB_MIN_POOL_RESERVE",
                                                       "25000"))
    """Ниже этого размера пул в справочник не берём.

    Сто тысяч стояло здесь из-за неверной оценки проскальзывания: пул
    меньше действительно не пережил бы своп на тысячу долларов, если
    считать его по формуле V2. С разделением V2 и V3 надобность в таком
    запасе отпала — глубина теперь оценивается ближе к правде, и мелкий
    пул отсеется сам, на расчёте, а не заранее и вслепую.

    Порог всё же не нулевой: пул на пару тысяч долларов чаще всего
    не торговая площадка, а декорация. Оборот такого пула проверяет
    отбор качества отдельно.
    """
    ...
    cex_venues: List[str] = field(default_factory=lambda: [
        ...
        "bitget", "htx", "coinex", "kraken", "bitfinex",
    ])
    ...
    dex_v3_depth_multiple: float = float(os.environ.get("ARB_V3_DEPTH", "10"))
    """Во сколько раз глубина у цены в пуле V3 больше, чем дала бы формула V2.

    Ликвидность в V3 стоит в выбранных поставщиком диапазонах, а не
    размазана по всей кривой. Из TVL точное значение не выводится, но
    порядок — единицы-десятки. Проверяется просто: сделать своп и
    сравнить полученное с расчётом в разборе связки.
    """

    dex_stable_depth_multiple: float = float(os.environ.get("ARB_STABLE_DEPTH", "5"))
    """Дополнительный запас глубины для пары из двух стейблов.

    Диапазоны там в доли процента, потому что цена оттуда не уходит.
    Своп USDT->DAI на тысячу долларов не двигает цену вовсе, и считать
    его по общей формуле — значит выдумывать издержку.
    """
...
# Площадки с сосредоточенной ликвидностью. Отличать их важно не ради
# красоты классификации: от этого напрямую зависит оценка глубины, а от
# неё — проходит связка в таблицу или нет. Uniswap V3 и всё, что построено
# на Algebra (Thena, QuickSwap V3, Ryze), позволяют поставщику задать
# диапазон цены; ликвидность собирается у текущей цены, и своп на $1000
# двигает её не на процент, а на сотые доли.
CONCENTRATED_MARKERS = ("v3", "v4", "algebra", "thena", "izumi", "maverick",
                        "ryze", "clmm", "slipstream")


def is_concentrated(dex_id: str) -> bool:
    """Сосредоточена ли ликвидность у цены (V3 и родня) или размазана (V2)."""
    v = normalize_dex(dex_id)
    return any(m in v for m in CONCENTRATED_MARKERS)


def is_stable(asset: str) -> bool:
    """Привязан ли актив к доллару.

    Пара из двух стейблов — особый случай для глубины: поставщики держат
    диапазоны в доли процента, потому что цена оттуда не уходит. Тот же
    TVL даёт на порядок больше исполнимого объёма у цены.
    """
    return (asset or "").strip().upper() in USD_LIKE
...
CEX_SYMBOL_LIMITS = {
    # Bitfinex отдавал 54 отказа из 79 пар, а на пятидесяти — 25 из 50.
    # Его квота не выдерживает даже умеренного параллелизма, и половина
    # запросов уходит в пустые ретраи. Двадцать пять пар отдаются чисто.
    "bitfinex": 25,
}
"""Персональные потолки числа пар, если общий велик для площадки."""
