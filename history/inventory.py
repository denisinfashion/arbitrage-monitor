...
class Inventory:
    ...
    def verdict(self, ticker: str) -> dict:
        ...
        if r["Пар"] < 2:
            return {
                "status": "мало пар",
                "text": f"{t} собирается, но торгуется только против "
                        f"{r['Торгуется с']}. Цикл USDT → … → USDT через него "
                        "не замыкается: нужна как минимум вторая пара.",
                "advice": "Связка появится, если у токена возникнет ещё один "
                          "пул с другим активом из набора.",
            }
