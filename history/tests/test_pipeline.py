...
def main() -> int:
    ...
    for _, r in table.head(4).iterrows():
        print(f"        {r['Связка']:34s} медиана {r['Маржа медиана, %']:+.4f}%  "
              f"макс {r['Маржа макс, %']:+.4f}%  в плюсе {r['Доля прибыльного времени, %']}%")

    cols = set(table.columns)
    need = {"Связка", "Ног", "Маржа макс, %", "Маржа медиана, %",
            "Доля прибыльного времени, %", "Площадки"}
    ...
    check("связка проходит через оба DEX",
          "biswap" in row["Площадки"] and "pancakeswap" in row["Площадки"],
          row["Площадки"])
    check("максимум маржи положителен", row["Маржа макс, %"] > 0,
          f"{row['Маржа макс, %']}%")
    check("доля прибыльного времени ≈ 25%",
          abs(row["Доля прибыльного времени, %"] - 25.0) < 3.0,
          f"{row['Доля прибыльного времени, %']}%")
